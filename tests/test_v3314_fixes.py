...
class TestIndependentBackupSources:
    ...
    def test_registry_total_source_count(self, router):
        ...
        v0.1.0-DEV tdx_mcp 新增 5 源（realtime/kline/screener/research/macro）→ 107。
        2026-09-23 东财 push2 族退役（删 8 个：em_push2/em_push2delay/em_push2ex/
        em_push2_clist/em_slist），剩余东财源降到 P999 → 100。"""
        ...
        #           → 107（v0.1.0-DEV tdx_mcp：realtime_quote + historical_kline 各一，
        #                  screener/research_report/macro_data 各一）
        #           → 100（2026-09-23 东财 push2 族退役：删 fund_flow:em_push2、
        #                  industry_comparison:em_push2、industry_quotes:em_push2、
